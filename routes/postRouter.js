...
const reportPostSchema = {
  reason: {
    ...
    required: true,
    custom: (value) => {
      const valid = ['spam', 'harassment', 'hate_speech', 'violence', 'nudity', 
                    'false_information', 'scam', 'copyright', 'self_harm', 'other'];
...
router.patch( "/post/:id/report", auth, validateObjectId('id'), validateBody(reportPostSchema), postCtrl.reportPost );

router.patch("/post/:id/hide", auth, validateObjectId('id'), postCtrl.hidePost);
